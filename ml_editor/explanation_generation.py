...
def get_explainer():
    ...
    we do not bother with serialising it.
...
def get_recommended_modification(simple_order, impact):
    ...
    simple_order : String
        Simplified operator
    impact : boolean
        Whether the change has positive or negative impact
...
def parse_explanations(exp_list):
    ...
    ----------
    exp_list : array-like
        explanations returned by LIME explainer

    Returns
    -------
        array of dictionaries containing user facing strings
    """
    parsed_exps = []
...
def get_recommendation_string_from_parsed_exps(exp_list):
    ...
    ----------
    exp_list : array-like
        array of dictionaries containing explanations

    Returns
    -------
        HTML displayable recommendation text
    """
    recommendations = []
    for i, feature_exp in enumerate(exp_list):
        ...
        if feature_exp["recommendation"] in ["Increase", "Decrease"]:
            font_color = "red"
        rec_str = """<font color={}>{}) {}</font>""".format(
            font_color,
            i+1,
            recommendation,
        )
